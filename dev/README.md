...
TLS at `dev.reverie.unkos.net` and forwarding to `:5173` inside the
workspace. Without the override the browser tries
`wss://dev.reverie.unkos.net:5173/` and the tunnel does not forward
that port.
...
REVERIE_DEV_HOSTS=dev.reverie.unkos.net REVERIE_DEV_HMR_CLIENT_PORT=443 npm run dev
```
